...
public class LoginAop {
     ...
     * 环绕增强
     */
    @Around("execution(* *..*Controller.*(..)) && @annotation(com.ns.util.IsLogin)")
    public Object isLogin(ProceedingJoinPoint proceedingJoinPoint){
        ...
        if(token != null){
            //查询redis
            //user = (Sys_user) redisTemplate.opsForValue().get(token);
        }
        ...
        for(int i = 0; i < args.length; i++){
            if(args[i] != null && args[i].getClass()==Sys_user.class){
                args[i] = user;
            }
        }
